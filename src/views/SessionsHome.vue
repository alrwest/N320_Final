<template>
   <h2 class ="conf-title"> Conference Session Information </h2>
  <div class="session-count">
     {{ sessionCount() }} out of 10 Sessions
    <div class="clearFilter-btn" v-if="filterExists" @click="clearFilters">Clear Filters</div>
  </div>
<div class="session-background">
  <session-data class ="session-card"
  v-for = "session in filteredSessions.length > 0 ? filteredSessions : sessions" :key="session.id" :session="session" @filter-by-presenter="presenterFilter"
...
export default { 
 ...
 data() {
  ...
  props: {
    ...
    tags: {type: Array},
    sTime: { type: Number, validator(value) { 
      return [8,10,13,15,20].includes(value)
    }},
    sDay: String,
    added: {default:false},
  ...
  methods: {
    //count sessions
      sessionCount() { 
        ...
        //display number of sessions : being shown out of 10
        console.log(this.filteredSessions.length);
        let sCount = 10;
        let fCount = this.filteredSessions.length;
        
        if (fCount > 0) { 
           //set counter equal to session count, if they're filtered
           console.log("How many sessions, with a filter?", fCount);
           sessionCounter = fCount;
        } else { 
          //if they're not filtered, display full count
          console.log("total sessions", sCount);
          sessionCounter = sCount;
        }
        return sessionCounter;
      },

      //filtering sessions
      presenterFilter(presenterName) { 
        console.log("presenter", presenterName);
        this.filteredSessions = this.sessions.filter((s) => s.presenter === presenterName);
        console.log("filtered by", presenterName);
        return this.filteredSessions;
      },

      categoryFilter(category) { 
       // console.log("category", category);
        this.filteredSessions = this.sessions.filter((s) => s.tags.includes(category));
  ...
  //check to see a filter exists, if so, display clear filter button
  computed: {
    filterExists() { 
  ...
  //using a watcher to watch for session count changes
  watch: { 
    sessionArray: { 
      handler(totalSessions) { 
        sCount = totalSessions.length;
        fCount = this.filteredSessions.length;
        console.log("How many sessions total?", sCount);
        console.log("How many sessions, with a filter?", fCount);
      }
    }
  }
