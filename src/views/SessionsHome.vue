<template>
  <div class="session-count">
    <h2> Conference Session Information </h2>
...
<div class="session-background">
  ...
  <!--<session-data class ="session-card"
  v-for = "session in filteredSessions" :key="session.id" :session="session" @filter-by-presenter="presenterFilter">
  </session-data>-->
</div>
 ...
 data() {
  ...
  methods: {
      sessionCount() { 
        ...
        //count how many session objects are being displayed, out of 10
        if (this.sessionArray.length == sessionLimit) { 
           //display number of sessions : being shown out of 10
           sessionCounter = this.sessionArray.length;
        } else { 
          sessionCounter = sessionLimit
        }
